...
--Made with ❤
-------
--Last Modified: Thuesday March 19th 2024 10:10:28 pm
-------
...
local function VERSION_CHECKER() -- DONT DELETE THIS TO BE INFORMED ABOUT NEW UPDATES
    ...
    PerformHttpRequest('https://api.github.com/repos/JustGodWork/jSync/releases/latest', function(status, response)
        ...
        if (currentVersion == latestVersion) then
            print("\n\n^7[^1jSync^7]^2 You are using the latest version.^7");
            return;
        end
...
AddEventHandler("onResourceStart", function(resource)
    if resource == GetCurrentResourceName() then
        ...
        WeatherService.sync();
        TimeService.sync();
        local hour, minute = TimeService.getCurrentTime();
...
RegisterNetEvent("jSync:onPlayerJoined", function()
    ...
    local hour, minute = TimeService.getCurrentTime();
    TriggerClientEvent("chat:addSuggestions", playerSource, commands);
    TriggerClientEvent("jSync:setWeather", playerSource, WeatherService.getCurrentWeather())
